...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public Response getEmployee(Integer employeeId) {
		...
		try {
			LOGGER.debug("Getting employee details:{}", employeeId);
			Optional<Employee> employee = employeeDao.findById(employeeId);
			if (employee.isPresent()) {
				res.setData(employee.get());
				res.setMessage(CommonConstants.Employee.EMPLOYEE_FOUND);
				res.setStatus(HttpStatus.OK);
				res.setMessage("Employee found");
			} else {
				res.setMessage(CommonConstants.Employee.EMPLOYEE_NOT_FOUND + employeeId);
			...
			res.setMessage(CommonConstants.Employee.EMPLOYEE_FAILED);
			e.getLocalizedMessage();
		}
		LOGGER.info("end");
		return res;
	}
	public Response getAllEmployee(Integer employeeId){
		LOGGER.info("Start");
		...
		try{
			...
			if (!employeeList.isEmpty()) {
				...
				Resp.setMessage(CommonConstants.Employee.EMPLOYEE_LIST_FOUND);
			} else {
				Resp.setStatus(HttpStatus.EXPECTATION_FAILED);
				Resp.setMessage(CommonConstants.Employee.EMPLOYEE_LIST_NOT_FOUND);
			...
			Resp.setStatus(HttpStatus.EXPECTATION_FAILED);
			Resp.setMessage(CommonConstants.Employee.EMPLOYEE_FAILED);
			e.getLocalizedMessage();
		}
		LOGGER.info("end");
		return Resp;
	...
	public Response getEmployee1(Integer employeeId) {
		LOGGER.info("Start");
		Response response = new Response();
		try {
	...
	@Override
	public Response getAllEmployee1(Integer employeeId) {
		return null;
	}

	public Response getAllEmployees1() {
		LOGGER.info("start");
		Response response = new Response();
